; (function () {
  ...
  const page = function () {
     ...
     * Options
     */
    window.addEventListener('__btpUserOptions__', function handler(event) {
      const options = JSON.parse(event.detail);
      enableDebug = options.enableDebug || false;
      if (enableDebug) {
        console.log('BlockTwitterPromoted | user options loaded: %o', options);
      }
      window.removeEventListener('__btpUserOptions__', handler);
  ...
  script.textContent = `;(${page}());`;
  script.async = false;
  setTimeout(async function () {
    script.remove();
    const options = (await optionsPromise).btpOptions || {};
    const event = new CustomEvent('__btpUserOptions__', { detail: JSON.stringify(options) });
